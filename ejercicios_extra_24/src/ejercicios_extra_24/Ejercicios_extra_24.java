...
public class Ejercicios_extra_24 {

    public static void main(String[] args) {
        Scanner leer = new Scanner(System.in);
        System.out.println("¿Cuántos números de la sucesión de Fibonacci deseas guardar?");
        int cant = leer.nextInt();
        int vector[] = new int[cant];
        CalcularFibonacci(vector,cant);
        for (int i = 0; i < cant; i++) {
            System.out.print("["+vector[i]+"] ");
        }
        System.out.println("");
    }
    public static void CalcularFibonacci(int vector[],int N){
        int num = 1;
        for (int i = 0; i < N; i++) {
            switch(i){
                case 0:
